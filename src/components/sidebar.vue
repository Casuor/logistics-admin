<template>
  <div class="sidebar-container" :class="{'collapse-width': isCollapse}">
    <div class="logo" :class="{'collapse-logo': isCollapse}" @click="
selectMenuItem({name:'index',title:'首页'})">
    ...
    <el-menu :default-active="this.$store.state.menus.editableTabsValue"
      ...
      <el-submenu :index="menu.name" v-for="menu in menuList" :key="menu.name">
        ...
        <router-link :to="item.path" v-for="item in menu.children" :key="item.name">
          <el-menu-item :index="item.name" @click="selectMenuItem(item)"><i :class="item.icon"></i>
            {{ item.title }}
          </el-menu-item>
        </router-link>
